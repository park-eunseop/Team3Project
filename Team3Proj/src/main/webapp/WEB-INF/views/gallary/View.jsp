		...
		<div class="wrap">
			<div class="post">
				<div class="post-headers">
					...
					<a id="modal_show" class="dropdown-item" href="#" data-toggle="modal" data-target="#moaModal"> 
				...
				<div class="thumbnail">
					<c:forEach var="file" items="${fitem}" varStatus="var">
						<c:if test="${item.notice_no == file.notice_no}">
				...
				<div class="post-footers">
					...
					<div class="text-container" style="padding-left:10px">
						...
						<c:if test="${item.notice_no == likeCount.notice_no }">
...
                    <button type="button" class="btn btn-secondary"  onclick="location.href='<c:url value="/Gallary/GallaryEdit.do?notice_no=${item.notice_no}"/>'">수정하기</button>
                    <button type="button" class="btn btn-primary" onclick="location.href='<c:url value="/Gallary/GallaryDelete.do?notice_no=${item.notice_no}"/>'">삭제하기</button>
                    <button type="button" class="btn btn-primary" id="close_modal" data-dismiss="modal">닫기</button>
                </div>
            </div>
        </div>
    </div>
...
function showComments(){
   $.ajax({
      ...
      data:{"notice_no":"${item.notice_no}","_csrf":"${_csrf.token}"},
      dataType:'json',
      type:'post',
...
function isDelete(){
   if(confirm("정말로 삭제 하시겠습니까?")){
      location.replace("<c:url value='/Gallary/delete.do?notice=${item.notice_no}'/>");
   }
}////////////isDelete
    ...
    $("#heart1").on("click", function () {
        ...
        var sendData = {'notice_no' : '${item.notice_no}','heart' : that.prop('name')};
        $.ajax({
